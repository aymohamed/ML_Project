...
def trainModel(model, X_train, Y_train, X_CV, Y_CV):
    ...
    model.fit_generator(dataGen.flow(X_train, Y_train, batch_size=batch_size), steps_per_epoch=len(X_train) / batch_size, epochs=epochs, verbose=1, callbacks=[earlyStopping, save_best_score, monitor_loss], validation_data=(X_CV, Y_CV))
